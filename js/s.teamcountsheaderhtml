...
(function (S) {
  ...
  S.renderPinnedSummaries = function () {
    ...
    var title = document.querySelector("#team-boards-dock .section-title"); if (title) title.textContent = "Pinned";
    var pinned = (S.teams.teams || []).filter(function (t) { return !!t.followMe; });
    if (!dock) return;
    dock.innerHTML = pinned.length ? '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(8.6rem,1fr));gap:0.35rem">' + pinned.map(function (t) { return S.teamSummaryHtml(t, { badge: true }); }).join("") + '</div>' : '<p class="muted">Pin a team pill to dock a summary here.</p>';
  };
  ...
  S.autoFormTeams = function () {
    ...
    ["STSO", "LTSO", "TSO"].forEach(function () {});
  ...
  S.bindTeamUI = function () {
    ...
    document.addEventListener("click", function (e) {
      ...
      else if (t.id === "btn-team-build") { S.teams.buildOpen = true; S.applyFollowMe(); S.renderPinnedSummaries(); if (S.updateStatus) S.updateStatus("Pinned board open"); }
